module Farscape
  ...
  class Representor < BasicObject
    ...
    def invoke(link_name, data_as_a_hash = nil)
      ...
      if link.nil?
        raise Farscape::UnknownTransition, "There was no link found for #{link_name} in #{@representor}"
      end
      if data_as_a_hash
        body = data_as_a_hash.to_json
        # If the link does not provide a proper method, that is a bug in the reprensation we got
        SimpleAgent.invoke(link.uri, {method: link.interface_method, body: body })
      else
        SimpleAgent.invoke(link.uri, {method: link.interface_method })
      end
    end
